...
export default function Student() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const studentRes = await axios.post(`https://e-leave-hub-backend.vercel.app/findstudent/${id}`);
                const acceptedRes = await axios.post(`https://e-leave-hub-backend.vercel.app/countupdate1/${id}`);
                const rejectedRes = await axios.post(`https://e-leave-hub-backend.vercel.app/countupdate0/${id}`);
                const pendingRes = await axios.post(`https://e-leave-hub-backend.vercel.app/countupdate-1/${id}`);
