...
export default function Student() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const studentRes = await axios.post(`http://localhost:5001/findstudent/${id}`);
                const acceptedRes = await axios.post(`http://localhost:5001/countupdate1/${id}`);
                const rejectedRes = await axios.post(`http://localhost:5001/countupdate0/${id}`);
                const pendingRes = await axios.post(`http://localhost:5001/countupdate-1/${id}`);
