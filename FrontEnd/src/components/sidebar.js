  ...
  const handleSubmit = async (e) => {
    ...
    if (user) {
      try {
        ...
        console.log(res.data); // Check what you get in response
  
        if (res.data.msg === "success") {
          alert("Successfully Uploaded..!");
          const imageUrl = res.data.imageUrl; // Use the relative path
          setImageUrl(imageUrl);
          user.profile = imageUrl; // Update user object
          sessionStorage.setItem("user", JSON.stringify(user)); // Save updated user
    ...
    <div className="col-md-1 col-lg-1 col-xl-1 col-sm-1 col-xs-1 menu">
      ...
      <div className="offcanvas offcanvas-start sidebar show" data-bs-scroll="true" data-bs-backdrop="false" tabIndex="-1" id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel" aria-modal="true" role="dialog">
        <div className="offcanvas-header">
          <h5 className="offcanvas-title" id="offcanvasScrollingLabel ">{user.name}</h5>
        ...
        <div className="offcanvas-body">
          <div className="mb-5">
            <div className="profileCircle">
              {isPopupVisible === true ? (
                    ...
                    <form onSubmit={handleSubmit}>
                      <div className="row">
                        <button className="btn-xs btn btn-light m-0 p-0">
                          <input type="file" name="myfile" accept="image/*" className=" form-control p-0 py-1 m-0" onChange={handleFileChange} />
                        </button>
                        <button type="submit" className=" btn btn-success m-0 p-0 py-1 mx-2">Upload</button>
                        <button onClick={hidePopup} className="btn-xs btn btn-danger p-0 py-1 m-0 mx-2">Close</button>
                      </div>
                    </form>
                  </div>
                </div>
                ...
                <>
                 {imageUrl !== "imageurl" ? (
                    <img src={`${process.env.REACT_APP_NODE_API}/${imageUrl}`} alt="profile" className="profileicon circular-image" />
                  ) : (
                    <img src={profile} alt="profile" className="profileicon circular-image" />
                  )}
          ...
          <Link className="row" onClick={renderDash}>Dashboard</Link>
          <Link className="row" to="/about">About</Link>
          <Link className="row" to="/contact">Contact</Link>
          <Link className="row" onClick={renderReq}>Request</Link>
          <Link className="row" onClick={logout}>Logout</Link>
        </div>
      </div>
    </div>
  );
