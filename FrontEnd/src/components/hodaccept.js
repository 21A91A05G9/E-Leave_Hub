...
export default function Hodaccept() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post(`https://e-leave-hub-backend.vercel.app/getdata/${id}`);
    ...
    const handleAccept = async (reqId) => {
        ...
        if (con) {
            try {
                await axios.put(`https://e-leave-hub-backend.vercel.app/set1/${reqId}`);
    ...
    const handleReject = async (reqId) => {
        ...
        if (con) {
            try {
                await axios.put(`https://e-leave-hub-backend.vercel.app/set0/${reqId}`);
